...
AS
/* Start of SELECT statement fetching data from t_tri2_properties */
SELECT 
    PROPERTY_ID,         -- Column from source table
    PROPERTY_KEY,        -- Column from source table
    PROPERTY_VALUE       -- Column from source table
FROM 
    t_tri2_properties cgu -- Source table alias for clarity
WHERE 
    1 = 1 -- Dummy condition, allows easy extension
    /* 
        The actual logic for the view was replaced with a simplified query.
        The detailed original logic (see below) was commented out.
    */
